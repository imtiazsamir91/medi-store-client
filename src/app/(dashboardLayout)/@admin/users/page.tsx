...
export default async function ManageUsersPage() {
  ...
  return (
    <div className="p-8 min-h-screen bg-transparent text-white selection:bg-[#8686AC]/30">
      
      {/* Header Section - Improved Typography & Button Glow */}
      <div className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6 animate-in fade-in slide-in-from-top-4 duration-1000">
        <div>
          ...
          <p className="text-[#8686AC]/70 mt-3 font-medium max-w-md leading-relaxed">
            Control access, monitor roles, and manage user permissions within your secure ecosystem.
          </p>
        </div>
        
        <button className="relative group px-10 py-4 rounded-2xl bg-gradient-to-r from-[#272757] via-[#505081] to-[#8686AC] text-white font-bold text-sm transition-all hover:shadow-[0_0_40px_rgba(134,134,172,0.5)] hover:scale-[1.05] active:scale-95 overflow-hidden">
  {/* Hover Shine Effect */}
  ...
  <span className="relative z-10 flex items-center gap-2 drop-shadow-sm">
    ...
    Create Account
      ...
      {/* Main Table Container - Enhanced Glassmorphism */}
      <div className="relative rounded-[2.5rem] border border-white/10 bg-[#0F0E47]/30 backdrop-blur-2xl shadow-[0_32px_64px_-16px_rgba(0,0,0,0.6)] overflow-hidden animate-in fade-in zoom-in-95 duration-700">
          ...
          <table className="w-full text-left border-separate border-spacing-y-2">
            ...
            <tbody className="divide-y divide-white/[0.03]">
              {users.map((user: any, index: number) => (
                <tr 
                  key={user.id} 
                  className="group hover:bg-white/[0.03] transition-all duration-500 ease-out"
                >
                    ...
                    <div className="flex items-center gap-5">
                      ...
                      <div className="relative">
                        ...
                        <div className="relative p-[1.5px] rounded-2xl bg-gradient-to-br from-white/20 to-transparent">
                          <div className="bg-[#1a194d] rounded-[14px] size-14 flex items-center justify-center text-xl font-black text-white group-hover:text-[#8686AC] transition-colors">
                            {user.name.charAt(0)}
                          </div>
                        </div>
                      </div>
                      <div className="space-y-1">
                        <p className="font-bold text-lg text-white group-hover:translate-x-1 transition-transform duration-300 tracking-tight">
                          {user.name}
                        </p>
                        <p className="text-xs text-[#8686AC] font-semibold tracking-wide uppercase opacity-60 italic">
                          {user.email}
                        </p>
                      </div>
                    </div>
                  ...
                  <td className="px-8 py-7">
                    <div className="inline-flex items-center gap-2.5 px-4 py-2 rounded-xl bg-white/[0.03] border border-white/5 text-[#8686AC] group-hover:border-[#505081] transition-all duration-500">
                  ...
                  <td className="px-8 py-7 text-right">
                    <div className="inline-block transform opacity-40 group-hover:opacity-100 group-hover:translate-x-[-4px] transition-all duration-300">
      ...
      {/* Footer System Info */}
      <div className="mt-8 flex justify-between items-center px-4 text-[10px] font-bold uppercase tracking-[0.3em] text-[#8686AC]/30">
